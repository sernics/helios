...
# Allow `python src/data/downloader.py` from the project root.
...
def _normalize_news_item(item: dict) -> dict:
    ...
    field is synthesised when absent so downstream code has a stable key for
    temporal alignment.
    """
    record = dict(item)

    if "providerPublishTime" not in record:
        pubdate = (record.get("content") or {}).get("pubDate")
        if pubdate:
            try:
